...
from dataset.build import build_dataset, build_dataset_sampler
from tokenizer.tokenizer_image.vq_vae_var_model import VQ_VAE_VAR_models
...
def main(args):
    ...
    if rank == 0:
        ...
        if args.enable_wandb:
            ...
            wandb.init(project=args.wandb_project, config=vars(args), name=f"llamagen-vq-train-{args.dataset}-Exp{experiment_index:03d}-{model_string_name}-{args.disc_type}-imsize{args.image_size}")
        logger.wandb_log = wandb.log if args.enable_wandb else None
    else:
        logger = create_logger(None)
        logger.wandb_log = None
    ...
    vq_model = VQ_VAE_VAR_models[args.vq_model](
    ...
    transform = transforms.Compose([
        transforms.Lambda(lambda pil_image: pil_image if args.interpolate_data else random_crop_arr(pil_image, args.image_size)),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5], inplace=True)
    ])
    
    dataset, sampler = build_dataset_sampler(args, world_size=dist.get_world_size(), rank=rank, shuffle=True, seed=args.global_seed, transform=transform)
    loader = DataLoader(
        dataset,
    ...
    dataset_size_min = torch.tensor(len(loader), device=device)
    dataset_size_max = dataset_size_min.clone()
    dataset_size_sum = dataset_size_min.clone()

    dist.all_reduce(dataset_size_min, op=dist.ReduceOp.MIN)
    dataset_size_min = dataset_size_min.cpu().item()
    
    dist.all_reduce(dataset_size_max, op=dist.ReduceOp.MAX)
    dataset_size_max = dataset_size_max.cpu().item()
    
    dist.all_reduce(dataset_size_sum, op=dist.ReduceOp.SUM)
    dataset_size_sum = dataset_size_sum.cpu().item()

    logger.info(f"Dataset contains {dataset_size_sum:,} images ({args.data_path}), min size: {dataset_size_min:,}, max size: {dataset_size_max:,}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (x, y) in enumerate(loader):
            ...
            if args.max_grad_norm != 0.0:
                ...
                torch.nn.utils.clip_grad_norm_(vq_model.module.parameters(), args.max_grad_norm)
            ...
            if train_steps % args.log_every == 0:
                ...
                if logger.wandb_log is not None:
                    logger.wandb_log({"train_loss": avg_loss, "steps_per_sec": steps_per_sec, "train_steps": train_steps, "train_fraction": train_steps / len(loader), 'dataset_size_min': dataset_size_min})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--interpolate-data", action='store_true', help="interpolate data into shape (512, 1024)")
    parser.add_argument("--wandb-project", type=str, default="AtmTokenizer", help="wandb project name")
    ...
    parser.add_argument("--vq-model", type=str, choices=list(VQ_VAE_VAR_models.keys()), default="VQ-VAE-VAR")
