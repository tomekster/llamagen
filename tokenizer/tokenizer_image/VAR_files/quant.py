"""
SOURCE: copied directly from VAR repository: models/quant.py
"""
...
class VectorQuantizer2(nn.Module):
    ...
    def forward(self, f_BChw: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, List[float]]:
        dtype = f_BChw.dtype
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            for si, pn in enumerate(self.v_patch_nums): # from small to large
                ...
                # tsternal: Updated to match VAE,
                ### f_hat := z_q == quantized feature map
                ### f_BChw := feature map before quantization
                ### f_no_grad := z.detach()
                
                # NOTE: tsternal, the first term and second term are the same value, just the first one is rescaled by Betha (default=0.25), and in the first term f_hat is supposed to be detached, while in the later one f is supposed to be detached
        ...
        # tsternal: From the original VAE encoder code
        ...
        # tsternal: In VAR code:
        # return f_hat, mean_vq_loss, usages
    
        # tsternal: Updated:
        # TODO: tsternal, convert usages for each patch to a single percentage usage
        
        mean_commit_loss: torch.Tensor = 0.0
        mean_entropy_loss: torch.Tensor = 0.0
        codebook_usage = 0.0
        return f_hat, (mean_vq_loss, mean_commit_loss, mean_entropy_loss, codebook_usage), usages
