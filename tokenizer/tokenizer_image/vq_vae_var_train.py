...
from dataset.build import build_dataset_sampler
...
def main(args):
    ...
    if rank == 0:
        ...
        if args.enable_wandb:
            ...
            wandb.init(project=args.wandb_project, config=vars(args), name=f"llamagen-vqvaevar-train-{args.dataset}-Exp{experiment_index:03d}-{model_string_name}-{args.disc_type}-imsize{args.image_size}")
    ...
    dataset_size = torch.tensor(len(loader), device=device)
    print(dataset_size)
    print(dist.all_reduce(dataset_size, op=dist.ReduceOp.MIN))
    
    # TODO: tsternal
    # dataset_size_min = dist.all_reduce(dataset_size, op=dist.ReduceOp.MIN).cpu().item()
    # dataset_size_max = dist.all_reduce(dataset_size, op=dist.ReduceOp.MAX).cpu().item()
    # dataset_size_sum = dist.all_reduce(dataset_size, op=dist.ReduceOp.SUM).cpu().item()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (x, y) in enumerate(loader):
            ...
            scaler.update()
            if args.ema:
                update_ema(ema, vq_model._orig_mod if args.compile else vq_model.module)
            ...
            if train_steps % args.log_every == 0:
                ...
                if logger.wandb_log is not None:
                    logger.wandb_log({"train_loss": avg_loss, "steps_per_sec": steps_per_sec, "train_steps": train_steps, "train_fraction": train_steps / len(loader)})
            ...
            if train_steps % args.ckpt_every == 0 and train_steps > 0:
                if rank == 0:
                    if args.compile:
                        model_weight = vq_model._orig_mod.state_dict()
                    else:
            ...
            # TODO: tsternal, bring back distributed 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vq-model", type=str, choices=list(VQ_VAE_VAR_models.keys()), default="VQ-VAR")
